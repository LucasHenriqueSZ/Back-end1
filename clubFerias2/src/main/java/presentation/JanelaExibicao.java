...
//                        dao.save(new Socio(nome, new Rg(documento)));
...
//                        dao.save(new Socio(nome, new Cpf(documento)));
//                        JOptionPane.showMessageDialog(null, "Membro gravado com sucesso!");
//                    } else {
...
//                try {
//                    Optional<Socio> member = dao.find(documentOrName);
//                    if (member.isPresent()) {
//                        JOptionPane.showMessageDialog(null, "Membro encontrado: \n" +
//                                "Nome: " + member.get().getNome() + "\n" +
//                                "Documento: " + member.get().getDocumento().toString() + "\n" +
//                                "Data de associação: " + member.get().getDataAssociacao() + "\n" +
//                                "Número da carteirinha: " + member.get().getCarteirinha());
//                    } else {
...
//                    }
//                    Optional<Socio> member = dao.find(numCarteirinha);
//                    if (member.isEmpty()) {
...
//                        String nome = JOptionPane.showInputDialog("Digite o nome do membro: ", member.get().getNome()).trim();
//                        String opcaoDocumento = JOptionPane.showInputDialog("Digite o tipo de documento: \n1 - RG\n2 - CPF",
//                                member.get().getDocumento().getDocumentSting().substring(0, 2).equals("RG") ? "1" : "2").trim();
//                        try {
//                            if (opcaoDocumento.equals("1")) {
//                                String documento = JOptionPane.showInputDialog("Digite o numero do RG: ", (
...
//
//                    List<Socio> list = dao.listAll(paginaAtual, quantidadePorPagina);
//                    String texto = "";
//                    for (Socio member : list) {
