...
public class MenuEspaco {
    ...
    private void editarEspaco(Scanner scanner) {
        try {
            ...
            String nome = scanner.nextLine();
            Optional<Espaco> espaco = EspacoDao.getInstance().buscarEspaco(nome);
            if (espaco.isPresent()) {
                ...
                EspacoDao.getInstance().atualizar(new Espaco(novoNome, novaDescricao, novaCapacidade, categorias), nome);
            } else {
    ...
    private void removerEspaco(Scanner scanner) {
        try {
            ...
            Optional<Espaco> espaco = EspacoDao.getInstance().buscarEspaco(nome);
            if (espaco.isPresent()) {
                EspacoDao.getInstance().deletar(espaco.get().getNome());
            ...
            if (espacos.isEmpty()) {
                ...
                for (Espaco espaco : espacos) {
                    at.addRow(espaco.getCodigo(), espaco.getNome(), espaco.getDescricao(), espaco.getLotacaoMaxima(),
                            getCategoriasNomes(espaco.getCategorias())).setTextAlignment(TextAlignment.CENTER);
                    at.addRule();
                }
                System.out.println(at.render());
                System.out.println("Total de espaços: " + espacos.size());
            }
    ...
    private String getCategoriasNomes(Set<String> categorias) {
        ...
        for (String codigoCategoria : categorias) {
            Optional<Categoria> categoria = CategoriaDao.getInstance().buscarCategoriaCodigo(codigoCategoria);
            if (categoria.isPresent()) {
